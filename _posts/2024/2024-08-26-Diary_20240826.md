...
- Linescan Camera DLL 보완
  - XML Config Page 작업 및 Simulation 진행중.
  - 카메라 초기화 관련 베이스 소스 참고.

■ SDV, UT iPAD 신작 관련

- iPAD CRD Inspection 모델 변경 관련 검토
  - Teaching 및 검사 처리 관련 10Tab 까지 처리 가능함.
  - 다만, Main 화면 결과 표시가 5Tab 까지만 되어 있어 수정 필요함.

[타겟일정]
■ FINEDNC, 압출캔 용접 라인

- ~ 08/30(금) : Linescan, Areascan Camera DLL 보완 완료 및 Lab. Test 진행 완료.
